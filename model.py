...
df = pd.read_csv(
    'D:\\Study\\ML\\Project\\Project_Documents\\new_simple_frontend-main\\Dataset\\dataset.csv')
df = df.dropna()
...
df = df.drop(['track_id', 'artists', 'album_name',
             'track_name', 'track_genre'], axis=1)
df2 = df.copy()
...
X_train_class, X_test_class, y_train_class, y_test_class = train_test_split(
    XClass, yClass, test_size=0.2, random_state=42)
# Xts = dftst
...
# 7
# rfr = RandomForestRegressor(random_state=42, max_leaf_nodes=20).fit(X_train_scaled,y_train)
